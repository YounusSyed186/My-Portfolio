...
const ShaderMaterial = ({
  ...
  const material = useMemo(() => {
    const prepareUniforms = (): { [key: string]: THREE.IUniform } => {
      const preparedUniforms: { [key: string]: THREE.IUniform } = {};

      // Convert uniforms to Three.js compatible format
      Object.entries(uniforms).forEach(([name, value]) => {
        if (Array.isArray(value.value)) {
          switch (value.value.length) {
            case 2:
              preparedUniforms[name] = {
                value: new THREE.Vector2().fromArray(value.value as number[]),
              };
              break;
            case 3:
              preparedUniforms[name] = {
                value: new THREE.Vector3().fromArray(value.value as number[]),
              };
              break;
            default:
              preparedUniforms[name] = { value: value.value };
              break;
          }
        } else {
          preparedUniforms[name] = { value: value.value };
    ...
    return new THREE.ShaderMaterial({
      vertexShader: `
        ...
        void main() {
          ...
          fragCoord = (position.xy + 1.0) * 0.5 * vec2(${size.width}, ${size.height});
          fragCoord.y = ${size.height.toFixed(1)} - fragCoord.y;
        }
      `,
